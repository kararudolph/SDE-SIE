# This uses compatible TMLE to estimate the complier stochastic direct effect and its variance, from the following publication
...
#make clever covariate
 ...
 #update Qy
  ...
  fitcz<- glm(z ~ -1 + a + nota + a:difqmza + nota:difqmza, weights=svywt*(1/tmpdat$ga), family="quasibinomial", data=tmpdat, offset=qlogis(gz[,1]))

  epsiloncz<-coef(fitcz)
  
  #update gz
  gzup<-cbind(plogis(qlogis(gz[,1]) + I(tmpdat$a==0)*epsiloncz[2] + I(tmpdat$a==0)*epsiloncz[4]*tmpdat$difqmza + I(tmpdat$a==1)*epsiloncz[1] + I(tmpdat$a==1)*epsiloncz[3]*tmpdat$difqmza),
    plogis(qlogis(gz[,2]) + epsiloncz[2] + epsiloncz[4]*tmpdat$difqmza),
    plogis(qlogis(gz[,3]) + epsiloncz[1] + epsiloncz[3]*tmpdat$difqmza)
    )
  ...
  eic2<-((2*a-1)/tmpdat$ga)*svywt*(tmpdat$qmz1 - tmpdat$qmz0) *(z-gzup[,1])
  #estimate denominator 
  psi2<-sum((gzup[,3]-gzup[,2])*svywt)/sum(svywt)
 
  eic3<-((qz[,2] - qz[,1])*svywt) - psi1
  eicdp1<-eic1 + eic2+ eic3

  eic1dp2<-((2*a-1)/tmpdat$ga)*svywt*(z - gz[,1])
  eic2dp2<-((gzup[,3] - gzup[,2])*svywt) - psi2
